...
var homeRootFolderPath = flag.String("homeRootFolder", "", "homeRootFolder to use")
...
func main() {
	...
	if *homeRootFolderPath != "" {
		...
		homeFolder := homeDir
		currentFolder := filepath.Join(homeFolder, *homeRootFolderPath)
		os.Setenv("IPFS_PATH", currentFolder)
		err := os.Chdir(currentFolder)
		if err != nil {
			panic(err)
		}
		ipfsPath = currentFolder
	} else if *repoPath != "" {
		ipfsPath = *repoPath
	} else {
		var err error
