...
func initFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) (_err error) {
	cctx := env.(*oldcmds.Context)
	currentFolder := cctx.ConfigRoot
	...
	if homeRootFolderStr != "" {
		...
		homeFolder := homeDir
		currentFolder := filepath.Join(homeFolder, homeRootFolderStr)
		os.Setenv("IPFS_PATH", currentFolder)
	...
	return doInit(os.Stdout, currentFolder, empty, profiles, conf)
}
